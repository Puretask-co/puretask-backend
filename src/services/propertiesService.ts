...
export async function createProperty(
  ...
  logger.info("property_created", { propertyId: result.rows[0].id, clientId });
...
export async function getPropertyById(
  ...
  const result = await query<Property>(queryText, params);
 ...
 */
export async function getClientProperties(clientId: string): Promise<Property[]> {
  const result = await query<Property>(
    `SELECT * FROM properties WHERE client_id = $1 ORDER BY label ASC`,
    [clientId]
  );
  return result.rows;
}
...
export async function updateProperty(
  ...
  logger.info("property_updated", { propertyId, clientId });
 ...
 */
export function calculateCleaningScore(property: Property): number {
  const now = new Date();
  let score = 100;
 ...
 */
export async function updateCleaningScore(
  propertyId: number,
...
export async function recalculateAllScores(): Promise<number> {
  ...
  logger.info("all_scores_recalculated", { updated, total: properties.rows.length });
...
export async function getRebookData(
  ...
  const suggestedSlots: Array<{ date: string; time: string }> = [];
  const now = new Date();
