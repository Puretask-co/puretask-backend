...
export interface ExtendedKpiMetrics extends DailyMetrics {
  // User metrics
  new_users: number;
  new_clients: number;
  new_cleaners: number;
 ...
 */
export async function createDailyKpiSnapshot(date?: string): Promise<KpiSnapshot> {
  // Use current_date if no date provided
  const targetDateRes = await query<{ date: string }>(
    `SELECT COALESCE($1::date, current_date)::text AS date`,
    [date ?? null]
  );
  const targetDate = targetDateRes.rows[0].date;

  // Aggregate metrics using a single efficient query
  const metricsResult = await query<DailyMetrics>(
    `
  ...
  logger.info("kpi_snapshot_created", {
    date: snapshot.date,
 ...
 */
export async function getExtendedDailyMetrics(date?: string): Promise<ExtendedKpiMetrics> {
  const targetDateRes = await query<{ date: string }>(
    `SELECT COALESCE($1::date, current_date)::text AS date`,
    [date ?? null]
  );
  ...
  const result = await query<ExtendedKpiMetrics>(
    `
      WITH job_metrics AS (
        SELECT
 ...
 */
export async function getKpiSnapshots(options: {
  startDate?: string;
  endDate?: string;
  limit?: number;
}): Promise<KpiSnapshot[]> {
 ...
 */
export async function getKpiGrowth(): Promise<{
  jobs_growth_pct: number;
  completed_growth_pct: number;
  current_week: DailyMetrics;
  previous_week: DailyMetrics;
}> {
