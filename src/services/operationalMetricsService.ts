...
export interface OperationalMetrics {
  ...
  jobs: {
    ...
    disputeRate: number; // per 100 jobs
  ...
  payouts: {
    ...
    failed: number;
    pending: number;
    withReconciliationFlags: number;
    reconciliationFlagRate: number; // percentage
  ...
  stripe: {
    ...
    webhookFailureRate: number; // percentage
  ...
  cleaners: {
    ...
    withLowFlexBadge: number;
    lowFlexBadgeRate: number; // percentage
...
export interface TrendMetrics {
  noShowRate: MetricTrend[];
  disputeRate: MetricTrend[];
  reconciliationFlagRate: MetricTrend[];
  webhookFailureRate: MetricTrend[];
  payoutSuccessRate: MetricTrend[];
}
...
export async function getOperationalMetrics(
  ...
  // Stripe metrics
  const stripeMetrics = await query<{
  ...
  // Cleaner metrics
  const cleanerMetrics = await query<{
    active: string;
    avg_reliability: string;
    with_low_flex: string;
    total_jobs: string;
  }>(
    `
      SELECT
...
export async function getMetricTrends(daysBack: number = 30): Promise<TrendMetrics> {
  const endDate = new Date();
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - daysBack);
  ...
  const payoutTrends = await query<{
    date: string;
    total: string;
    with_flags: string;
    paid: string;
  ...
  const webhookTrends = await query<{
    date: string;
    total: string;
    failed: string;
  }>(
  ...
  // Build trend arrays
  const noShowRate: MetricTrend[] = jobTrends.rows.map((row) => ({
    date: row.date,
    value:
  ...
  const disputeRate: MetricTrend[] = jobTrends.rows.map((row) => ({
    date: row.date,
    value:
      parseInt(row.total, 10) > 0
  ...
  const reconciliationFlagRate: MetricTrend[] = payoutTrends.rows.map((row) => ({
    date: row.date,
    value:
      parseInt(row.total, 10) > 0
  ...
  const webhookFailureRate: MetricTrend[] = webhookTrends.rows.map((row) => ({
    date: row.date,
    value:
      parseInt(row.total, 10) > 0
  ...
  const payoutSuccessRate: MetricTrend[] = payoutTrends.rows.map((row) => ({
    date: row.date,
    value:
      parseInt(row.total, 10) > 0
...
export async function getSystemHealthSnapshot(): Promise<SystemHealthSnapshot> {
  ...
  const webhookHealth = await query<{ total: string; failed: string }>(
    `
      SELECT
        COUNT(*) AS total,
        COUNT(*) FILTER (WHERE processed = false) AS failed
  ...
  let webhookStatus: "ok" | "degraded" | "error" = "ok";
  ...
  const payoutHealth = await query<{ pending: string; oldest_hours: string }>(
    `
      SELECT
        COUNT(*) AS pending,
  ...
  let payoutStatus: "ok" | "degraded" | "error" = "ok";
  ...
  // Stuck jobs check
  const stuckJobs = await query<{ count: string }>(
    `
  ...
  let jobStatus: "ok" | "degraded" | "error" = "ok";
  if (stuckCount > 10) {
    jobStatus = "error";
    if (overallStatus !== "critical") overallStatus = "critical";
  ...
  const reconFlags = await query<{ count: string }>(
    `
      SELECT COUNT(*) AS count
  ...
  let reconStatus: "ok" | "warning" | "error" = "ok";
