...
export function getEmailSubject(type: NotificationType): string {
...
export function getEmailBody(type: NotificationType, data: Record<string, unknown>): string {
  const jobId = data.jobId as string || "N/A";
  const clientName = data.clientName as string || "Customer";
  const cleanerName = data.cleanerName as string || "Your cleaner";
  const address = data.address as string || "";
  const creditAmount = data.creditAmount as number || 0;
...
export function getSmsBody(type: NotificationType, data: Record<string, unknown>): string {
  const jobId = (data.jobId as string)?.slice(0, 8) || "job";
  const cleanerName = data.cleanerName as string || "Your cleaner";
...
export function getPushTitle(type: NotificationType): string {
...
export function getPushBody(type: NotificationType, data: Record<string, unknown>): string {
  const jobId = (data.jobId as string)?.slice(0, 8) || "";
  const cleanerName = data.cleanerName as string || "Your cleaner";
  const creditAmount = data.creditAmount as number || 0;
