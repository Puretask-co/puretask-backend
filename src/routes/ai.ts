  ...
  authedHandler(async (req: AuthedRequest, res: Response) => {
    try {
      ...
      if (result.rows.length === 0) {
        res
          .status(404)
          .json({ error: { code: "NOT_FOUND", message: "Cleaner profile not found" } });
        return;
      }
...
aiRouter.post(
  ...
  validateBody(clientResponseSchema),
  authedHandler(async (req: AuthedRequest, res: Response) => {
    try {
      const userId = req.user!.id;
      ...
      const bookingResult = await query(`SELECT client_id, cleaner_id FROM jobs WHERE id = $1`, [
        booking_id,
      ]);
...
aiRouter.post(
  "/generate-response",
  validateBody(generateResponseSchema),
  authedHandler(async (req: AuthedRequest, res: Response) => {
    try {
      const userId = req.user!.id;
