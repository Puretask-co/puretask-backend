...
router.get(
  ...
  authedHandler(async (req: AuthedRequest, res: Response) => {
    try {
      ...
      const result = await query(`SELECT * FROM admin_settings WHERE setting_key = $1`, [key]);
...
router.put(
  ...
  authedHandler(async (req: AuthedRequest, res: Response) => {
    try {
      ...
      const existing = await query(`SELECT * FROM admin_settings WHERE setting_key = $1`, [key]);
