...
describe("Auth API Integration Tests", () => {
  ...
  describe("GET /auth/me", () => {
    ...
    beforeAll(async () => {
      const registerResponse = await request(app).post("/auth/register").send(testUser).expect(201);
      authToken = registerResponse.body.token;
      if (!authToken || typeof authToken !== "string") {
        throw new Error(
          "Registration failed: no token in response. Body: " +
    ...
    it("should reject request with malformed authorization header", async () => {
      await request(app).get("/auth/me").set("Authorization", "InvalidFormat").expect(401);
    });
  });
});
