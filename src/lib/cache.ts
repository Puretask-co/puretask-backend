...
export class CacheService {
   ...
   * Set value in cache
   */
  static async set<T>(key: string, value: T, ttl: number = DefaultTTL.MEDIUM): Promise<void> {
    const redis = this.getRedis();
    if (!redis) return;
...
export function Cacheable(options: CacheOptions = {}) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      // Cache result
      await CacheService.set(cacheKey, result, options.ttl || DefaultTTL.MEDIUM);
...
export class CacheInvalidation {
  ...
  static async invalidateJob(jobId: string, clientId?: string, cleanerId?: string): Promise<void> {
    const promises = [CacheService.del(CacheKeys.JOB_DETAILS(jobId))];
...
export class CacheWarming {
  ...
  static async warmFeaturedCleaners(fetchFn: () => Promise<any>): Promise<void> {
    try {
      ...
      await CacheService.set(CacheKeys.FEATURED_CLEANERS(), data, DefaultTTL.LONG);
  ...
  static async warmAdminStats(fetchFn: () => Promise<any>): Promise<void> {
    try {
      const data = await fetchFn();
      await CacheService.set(CacheKeys.ADMIN_STATS(), data, DefaultTTL.SHORT);
