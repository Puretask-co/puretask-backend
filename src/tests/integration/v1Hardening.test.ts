...
import { addLedgerEntry, getUserBalance, escrowCreditsWithTransaction } from "../../services/creditsService";
...
// Test user IDs
const TEST_CLIENT_ID = "test-client-v1-hardening-001";
const TEST_CLEANER_ID = "test-cleaner-v1-hardening-001";
...
describe("V1 Hardening: Environment Guards", () => {
  ...
  describe("V1 Hardening: Ledger Idempotency", () => {
    ...
    beforeEach(async () => {
      ...
      await query(
        `INSERT INTO jobs (id, client_id, status, credit_amount, scheduled_start_at, scheduled_end_at, address, estimated_hours)
         VALUES ($1, $2, 'requested', 1000, NOW() + INTERVAL '2 hours', NOW() + INTERVAL '4 hours', '123 Test St', 2)
  ...
  it("should prevent duplicate escrow entries", async () => {
    // First escrow
    await escrowCreditsWithTransaction({ clientId: TEST_CLIENT_ID, jobId: testJobId, creditAmount: 1000 });

    // Attempt duplicate escrow (should be idempotent)
    const entry2 = await escrowCreditsWithTransaction({ clientId: TEST_CLIENT_ID, jobId: testJobId, creditAmount: 1000 });
...
describe("V1 Hardening: Atomic Job Completion", () => {
  ...
  beforeEach(async () => {
    ...
    testJobId = crypto.randomUUID();
    await query(
      `INSERT INTO jobs (id, client_id, cleaner_id, status, credit_amount, scheduled_start_at, scheduled_end_at, address, actual_end_at, estimated_hours)
       VALUES ($1, $2, $3, 'awaiting_approval', 500, NOW() - INTERVAL '2 hours', NOW() - INTERVAL '1 hour', '123 Test St', NOW() - INTERVAL '1 hour', 2)
       ON CONFLICT (id) DO NOTHING`,
      [testJobId, TEST_CLIENT_ID, TEST_CLEANER_ID]
    );

    // Escrow credits
    await escrowCreditsWithTransaction({ clientId: TEST_CLIENT_ID, jobId: testJobId, creditAmount: 500 });
  });
  ...
  it("should atomically complete job, release credits, and record earnings", async () => {
    ...
    expect(parseInt(afterPayouts.rows[0].count)).toBeGreaterThanOrEqual(parseInt(beforePayouts.rows[0].count));
  });
});
