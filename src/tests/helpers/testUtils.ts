...
export async function cleanupTestData(): Promise<void> {
  ...
  const cleanupQueries = [
    ...
    `DELETE FROM cleaner_teams WHERE owner_cleaner_id IN (SELECT id FROM users WHERE email LIKE '%@test.puretask.com')`,
    `DELETE FROM cleaner_goals WHERE cleaner_id IN (SELECT id FROM users WHERE email LIKE '%@test.puretask.com')`,
