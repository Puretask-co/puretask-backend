...
**Where it lives:** `src/lib/n8nClient.ts` (triggerN8nWorkflow, getWorkflowExecutions, getWorkflowStatus, listWorkflows, setWorkflowActive, getExecutionDetails, triggerN8nWorkflowWithRetry, isN8nConfigured, testN8nConnection); `src/integrations/n8n.ts` re-exports  
**Why document:** How the backend triggers n8n workflows and checks status; when we use it (e.g. after events).
...
**Technical:** The n8n client in PureTask is the code that talks to the n8n API so we can run workflows (automations) from the backend. n8n is a workflow automation tool; we call it when we want to trigger a workflow (e.g. after a job event). **API:** We use the n8n Cloud API (base URL https://puretask.app.n8n.cloud/api/v1). All requests use N8N_API_KEY in the X-N8N-API-KEY header. **Functions:** triggerN8nWorkflow(workflowId, data) POSTs to /workflows/{id}/execute with body data and returns { executionId }; getWorkflowExecutions(workflowId, { limit, status }) GETs executions list; getWorkflowStatus(workflowId) GETs workflow metadata (id, name, active, createdAt, updatedAt); listWorkflows() GETs all workflows; setWorkflowActive(workflowId, active) PUTs to toggle active; getExecutionDetails(executionId) GETs one execution; triggerN8nWorkflowWithRetry(workflowId, data, maxRetries) calls triggerN8nWorkflow with retry loop (1s, 2s, 3s delays); isN8nConfigured() returns true if N8N_API_KEY and N8N_MCP_SERVER_URL are set; testN8nConnection() returns { connected, workflows?, error? }. Internal helper n8nApiRequest(endpoint, { method, body }) does fetch with auth and throws on non-OK; all public functions log on success or failure.
...
**Technical:** `src/lib/n8nClient.ts` defines all functions. `src/integrations/n8n.ts` re-exports (currently references sendN8nWebhook, sendN8nEvent which may live elsewhere or be legacy—the main n8nClient has triggerN8nWorkflow, listWorkflows, etc.). Callers: any code that needs to trigger an n8n workflow or check status should import from n8nClient (or integrations/n8n). Event system or notification flow may call triggerN8nWorkflow or triggerN8nWorkflowWithRetry after publishing an event (see FOUNDER_EVENTS). Tests: src/tests/integration/externalServices.test.ts mocks or calls triggerN8nWorkflow. Env: N8N_API_KEY (required for API calls), N8N_MCP_SERVER_URL (used by isN8nConfigured).
