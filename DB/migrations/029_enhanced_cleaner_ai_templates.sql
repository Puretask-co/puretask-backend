-- Migration 029: Enhanced Cleaner AI Templates & Quick Responses
...
SELECT 
  ...
  '["client_name", "property_type", "rooms_cleaned", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;

-- Follow-up Request for Reviews
INSERT INTO cleaner_ai_templates (cleaner_id, template_type, template_name, template_content, variables, is_default, is_active)
SELECT 
  user_id,
  'review_request',
...
SELECT 
  ...
  '["client_name", "original_date", "original_time", "reason", "alternative_dates", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  '["client_name", "minutes", "reason", "new_eta", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  '["client_name", "time", "instructions", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  '["client_name", "amount", "property_type", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;

-- Holiday/Vacation Auto-Reply
INSERT INTO cleaner_ai_templates (cleaner_id, template_type, template_name, template_content, variables, is_default, is_active)
SELECT 
  user_id,
  'vacation_reply',
  ...
  '["client_name", "start_date", "end_date", "backup_contact", "cleaner_name"]'::jsonb,
  false,
  false
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  '["client_name", "weather_condition", "scheduled_time", "delay_minutes", "cleaner_name"]'::jsonb,
  false,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  '["client_name", "property_type", "date", "time", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;

-- Post-Service Issue Resolution
INSERT INTO cleaner_ai_templates (cleaner_id, template_type, template_name, template_content, variables, is_default, is_active)
SELECT 
  user_id,
  'issue_resolution',
  ...
  '["client_name", "issue", "solution", "cleaner_name"]'::jsonb,
  true,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  '["client_name", "referred_name", "referral_discount", "cleaner_name"]'::jsonb,
  false,
  true
FROM cleaner_profiles
ON CONFLICT DO NOTHING;
...
SELECT 
  user_id,
  'payment',
  ARRAY['payment', 'pay', 'card', 'cash', 'venmo', 'paypal', 'zelle'],
...
SELECT 
  user_id,
  'pets',
  ARRAY['pet', 'dog', 'cat', 'animal', 'puppy'],
...
SELECT 
  user_id,
  'supplies',
  ARRAY['supplies', 'products', 'bring', 'provide', 'cleaning products', 'equipment'],
...
SELECT 
  user_id,
  'special_requests',
  ARRAY['special', 'extra', 'additional', 'custom', 'specific'],
...
SELECT 
  user_id,
  'access',
  ARRAY['parking', 'access', 'key', 'gate', 'code', 'enter', 'lockbox'],
...
SELECT 
  user_id,
  'issues',
  ARRAY['issue', 'problem', 'concern', 'complaint', 'not satisfied', 'disappointed'],
...
SELECT 
  user_id,
  'tipping',
  ARRAY['tip', 'gratuity', 'extra', 'appreciation'],
...
SELECT 
  user_id,
  'frequency',
  ARRAY['weekly', 'biweekly', 'monthly', 'recurring', 'regular', 'schedule'],
...
SELECT 
  user_id,
  'eco_friendly',
  ARRAY['eco', 'green', 'natural', 'organic', 'chemical', 'safe', 'toxic'],
...
SELECT 
  user_id,
  'time_estimate',
  ARRAY['how long', 'duration', 'time', 'hours', 'minutes'],
...
SELECT 
  user_id,
  'background',
  ARRAY['background', 'check', 'verified', 'insured', 'bonded', 'trust'],
...
SELECT 
  user_id,
  'move_cleaning',
  ARRAY['move', 'moving', 'move-in', 'move-out', 'empty', 'vacant'],
...
SELECT 
  user_id,
  'same_day',
  ARRAY['same day', 'today', 'urgent', 'emergency', 'asap', 'last minute'],
...
SELECT 
  user_id,
  'whats_included',
  ARRAY['include', 'included', 'standard', 'basic', 'what do you clean', 'service'],
...
BEGIN
  SELECT COUNT(*) INTO template_count FROM cleaner_ai_templates;
  SELECT COUNT(*) INTO response_count FROM cleaner_quick_responses;
  
  RAISE NOTICE '✅ Enhanced templates and responses added successfully!';
  RAISE NOTICE 'Total Templates: %', template_count;
  RAISE NOTICE 'Total Quick Responses: %', response_count;
END $$;
