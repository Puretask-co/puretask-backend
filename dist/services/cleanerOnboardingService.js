...
exports.updateCleanerAvailability = updateCleanerAvailability;
exports.getCleanerAvailability = getCleanerAvailability;
exports.getCleanerProfile = getCleanerProfile;
const stripe_1 = __importDefault(require("stripe"));
const client_1 = require("../db/client");
const env_1 = require("../config/env");
const logger_1 = require("../lib/logger");
const APP_URL = env_1.env.APP_URL;
 ...
 */
async function createStripeConnectAccount(cleanerId) {
    // Get cleaner info with profile
    const userResult = await (0, client_1.query)(`SELECT u.*, cp.stripe_connect_id, cp.tier, cp.base_rate_cph, 
     ...
     WHERE u.id = $1 AND u.role = 'cleaner'`, [cleanerId]);
    ...
    logger_1.logger.info("stripe_connect_account_created", {
        cleanerId,
 ...
 */
async function getStripeConnectStatus(cleanerId) {
    const userResult = await (0, client_1.query)(`SELECT stripe_connect_id FROM cleaner_profiles WHERE user_id = $1`, [cleanerId]);
    ...
    try {
        ...
        logger_1.logger.error("stripe_connect_status_failed", {
            cleanerId,
            accountId,
            error: error.message,
        });
 ...
 */
async function getStripeDashboardLink(cleanerId) {
    const userResult = await (0, client_1.query)(`SELECT stripe_connect_id FROM cleaner_profiles WHERE user_id = $1`, [cleanerId]);
    if (userResult.rows.length === 0 || !userResult.rows[0].stripe_connect_id) {
        throw new Error("No Stripe Connect account found");
    }
    const loginLink = await stripe.accounts.createLoginLink(userResult.rows[0].stripe_connect_id);
    return loginLink.url;
}
/**
 * Update cleaner profile/rates
 */
async function updateCleanerProfile(cleanerId, updates) {
 ...
 */
async function updateCleanerAvailability(cleanerId, availability) {
    await (0, client_1.query)(`
    ...
    logger_1.logger.info("cleaner_availability_updated", { cleanerId });
}
/**
 * Get cleaner availability
 */
async function getCleanerAvailability(cleanerId) {
    const result = await (0, client_1.query)(`SELECT availability_json FROM cleaner_availability WHERE cleaner_id = $1`, [cleanerId]);
    return result.rows[0]?.availability_json || {};
}
/**
 * Get cleaner profile with stats
 */
async function getCleanerProfile(cleanerId) {
    const result = await (0, client_1.query)(`SELECT u.*, cp.tier, cp.base_rate_cph, cp.deep_addon_cph, cp.moveout_addon_cph,
