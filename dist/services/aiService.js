...
exports.generateDisputeSuggestion = generateDisputeSuggestion;
exports.queueChecklistGeneration = queueChecklistGeneration;
exports.queueDisputeSuggestion = queueDisputeSuggestion;
 ...
 */
async function invokeModel(systemPrompt, userMessage, jsonMode = true) {
    if (!AI_CONFIG.OPENAI_API_KEY) {
        logger_1.logger.warn("ai_api_key_missing", { fallback: "using_mock_response" });
    ...
    try {
        ...
        logger_1.logger.error("ai_model_invocation_failed", { error: err.message });
        throw err;
    }
}
// ============================================
 ...
 */
async function generateChecklist(input) {
    const userMessage = `
Property Details:
...
Generate a detailed cleaning checklist for this property.`;
    try {
        ...
        logger_1.logger.info("ai_checklist_generated", {
            bedrooms: input.bedrooms,
        ...
        logger_1.logger.error("ai_checklist_failed", { error: err.message });
        return generateFallbackChecklist(input);
    }
}
/**
 ...
 */
function generateFallbackChecklist(input) {
    const baseHours = {
        basic: 0.5,
        deep: 0.75,
        moveout: 1.0,
 ...
 */
async function generateDisputeSuggestion(input) {
    const userMessage = `
Dispute Details:
- Job ID: ${input.job_id}
...
Analyze this dispute and provide a recommendation.`;
    try {
        ...
        const aiResult = result;
        logger_1.logger.info("ai_dispute_suggestion_generated", {
            jobId: input.job_id,
        ...
        logger_1.logger.error("ai_dispute_suggestion_failed", { error: err.message });
        return generateFallbackDisputeSuggestion(input);
    }
}
/**
 ...
 */
function generateFallbackDisputeSuggestion(input) {
    const factors = [];
 ...
 */
async function queueChecklistGeneration(jobId, input) {
    await (0, queue_1.enqueue)(queue_1.QUEUE_NAMES.AI_CHECKLIST, { jobId, input });
    logger_1.logger.debug("checklist_generation_queued", { jobId });
}
/**
 * Queue dispute suggestion (non-blocking)
 */
async function queueDisputeSuggestion(disputeId, input) {
    await (0, queue_1.enqueue)(queue_1.QUEUE_NAMES.AI_DISPUTE, { disputeId, input });
    logger_1.logger.debug("dispute_suggestion_queued", { disputeId });
}
