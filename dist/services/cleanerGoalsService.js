...
exports.createDefaultMonthlyGoals = createDefaultMonthlyGoals;
exports.getCleanerGoals = getCleanerGoals;
exports.updateGoalProgress = updateGoalProgress;
exports.getRouteSuggestions = getRouteSuggestions;
exports.getReliabilityBreakdown = getReliabilityBreakdown;
 ...
 */
async function createGoal(cleanerId, month, goalType, targetValue, rewardCredits) {
    ...
    logger_1.logger.info("goal_created", { cleanerId, goalType, month: monthStr, targetValue });
    return result.rows[0];
}
/**
 * Auto-create default monthly goals for cleaner
 */
async function createDefaultMonthlyGoals(cleanerId, month) {
    const targetMonth = month || new Date();
    // Check cleaner's level based on past performance
    const historyResult = await (0, client_1.query)(`
    ...
    if (avgJobs >= 40)
        template = GOAL_TEMPLATES.ELITE;
    else if (avgJobs >= 25)
        template = GOAL_TEMPLATES.PRO;
    else if (avgJobs >= 15)
        template = GOAL_TEMPLATES.REGULAR;
    const goal = await createGoal(cleanerId, targetMonth, "jobs", template.target_jobs, template.reward_credits);
    return [goal];
}
/**
 * Get cleaner's goals for a month
 */
async function getCleanerGoals(cleanerId, month) {
 ...
 */
async function updateGoalProgress(cleanerId) {
 ...
 */
async function checkAndAwardGoals(cleanerId, month) {
    const result = await (0, client_1.query)(`
    ...
    for (const goal of result.rows) {
        ...
        // Mark as awarded
        await (0, client_1.query)(`UPDATE cleaner_goals SET is_awarded = true, awarded_at = NOW() WHERE id = $1`, [goal.id]);
        logger_1.logger.info("goal_achieved", {
            cleanerId,
            goalId: goal.id,
 ...
 */
async function getRouteSuggestions(cleanerId, date) {
 ...
 */
function estimateTravelTime(job1, job2) {
    // If we have coordinates, calculate distance
    if (job1.latitude && job1.longitude && job2.latitude && job2.longitude) {
        const distance = haversineDistance(job1.latitude, job1.longitude, job2.latitude, job2.longitude);
 ...
 */
async function getReliabilityBreakdown(cleanerId) {
    // Get current score and tier
    const profileResult = await (0, client_1.query)(`SELECT reliability_score::text, tier FROM cleaner_profiles WHERE user_id = $1`, [cleanerId]);
