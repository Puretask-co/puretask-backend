...
exports.computeReliabilityScoreFromStats = computeReliabilityScoreFromStats;
exports.getTierFromScore = getTierFromScore;
exports.updateCleanerReliability = updateCleanerReliability;
exports.recalcAllCleanersReliability = recalcAllCleanersReliability;
exports.getCleanerReliabilityInfo = getCleanerReliabilityInfo;
 ...
 */
async function computeCleanerStats(cleanerId) {
 ...
 */
function computeReliabilityScoreFromStats(stats, photoComplianceBonus = false) {
    ...
    if (photoComplianceBonus) {
        score += creditEconomyService_1.CREDIT_ECONOMY_CONFIG.PHOTO_COMPLIANCE_BONUS;
    }
...
async function updateCleanerReliability(cleanerId, reason = "job_completed") {
    ...
    if (tierLocked && newTierIndex < previousTierIndex) {
        ...
        logger_1.logger.info("tier_demotion_blocked_by_lock", {
            cleanerId,
    ...
    logger_1.logger.info("reliability_updated", {
        cleanerId,
        previousScore,
        newScore,
        previousTier,
 ...
 */
async function recalcAllCleanersReliability() {
    const cleaners = await (0, client_1.query)(`SELECT user_id FROM cleaner_profiles`);
    const updates = [];
    let processed = 0;
    let failed = 0;
    for (const cleaner of cleaners.rows) {
        try {
            ...
            logger_1.logger.error("reliability_bulk_failed", {
    ...
    logger_1.logger.info("reliability_bulk_completed", {
        processed,
        failed,
 ...
 */
async function getCleanerReliabilityInfo(cleanerId) {
    const profileResult = await (0, client_1.query)(`SELECT reliability_score, tier FROM cleaner_profiles WHERE user_id = $1`, [cleanerId]);
