...
exports.setDayAvailability = setDayAvailability;
exports.setWeeklyAvailability = setWeeklyAvailability;
exports.clearDayAvailability = clearDayAvailability;
...
exports.addServiceArea = addServiceArea;
exports.deleteServiceArea = deleteServiceArea;
exports.getPreferences = getPreferences;
exports.setPreferences = setPreferences;
exports.isCleanerAvailable = isCleanerAvailable;
exports.isCleanerAvailableForSlot = isCleanerAvailableForSlot;
exports.getAvailableCleaners = getAvailableCleaners;
exports.cleanerServesZipCode = cleanerServesZipCode;
exports.getCleanerSchedule = getCleanerSchedule;
 ...
 */
async function getWeeklyAvailability(cleanerId) {
    const result = await (0, client_1.query)(`SELECT * FROM cleaner_availability WHERE cleaner_id = $1 ORDER BY day_of_week`, [cleanerId]);
    return result.rows;
}
/**
 * Set cleaner's availability for a specific day
 */
async function setDayAvailability(cleanerId, dayOfWeek, startTime, endTime, isAvailable = true) {
    ...
    logger_1.logger.info("cleaner_availability_set", {
        cleanerId,
        dayOfWeek,
        startTime,
        endTime,
 ...
 */
async function setWeeklyAvailability(cleanerId, slots) {
    // Delete existing availability
    await (0, client_1.query)(`DELETE FROM cleaner_availability WHERE cleaner_id = $1`, [cleanerId]);
    // Insert new slots
    const results = [];
    for (const slot of slots) {
        const result = await setDayAvailability(cleanerId, slot.dayOfWeek, slot.startTime, slot.endTime, slot.isAvailable);
        results.push(result);
    }
    logger_1.logger.info("cleaner_weekly_availability_set", {
        cleanerId,
        slots: slots.length,
    });
    return results;
 ...
 */
async function clearDayAvailability(cleanerId, dayOfWeek) {
    await (0, client_1.query)(`DELETE FROM cleaner_availability WHERE cleaner_id = $1 AND day_of_week = $2`, [cleanerId, dayOfWeek]);
}
// ============================================
// Time Off
// ============================================
 ...
 */
async function getTimeOff(cleanerId, futureOnly = true) {
    ...
    const result = await (0, client_1.query)(queryText, [cleanerId]);
    return result.rows;
}
/**
...
async function addTimeOff(params) {
    ...
    logger_1.logger.info("cleaner_time_off_added", {
        cleanerId,
        startDate,
        endDate,
        allDay,
 ...
 */
async function deleteTimeOff(cleanerId, timeOffId) {
    await (0, client_1.query)(`DELETE FROM cleaner_time_off WHERE id = $1 AND cleaner_id = $2`, [timeOffId, cleanerId]);
    logger_1.logger.info("cleaner_time_off_deleted", { cleanerId, timeOffId });
}
// ============================================
// Service Areas
// ============================================
 ...
 */
async function getServiceAreas(cleanerId) {
    const result = await (0, client_1.query)(`SELECT * FROM cleaner_service_areas WHERE cleaner_id = $1 ORDER BY created_at`, [cleanerId]);
    return result.rows;
}
/**
 * Add a service area for a cleaner
 */
async function addServiceArea(params) {
    ...
    logger_1.logger.info("cleaner_service_area_added", { cleanerId, zipCode, city, state });
    return result.rows[0];
}
/**
 * Delete a service area
 */
async function deleteServiceArea(cleanerId, areaId) {
    await (0, client_1.query)(`DELETE FROM cleaner_service_areas WHERE id = $1 AND cleaner_id = $2`, [areaId, cleanerId]);
}
// ============================================
// Preferences
// ============================================
 ...
 */
async function getPreferences(cleanerId) {
    const result = await (0, client_1.query)(`SELECT * FROM cleaner_preferences WHERE cleaner_id = $1`, [cleanerId]);
    return result.rows[0] ?? null;
}
/**
 * Set/update cleaner preferences
 */
async function setPreferences(cleanerId, prefs) {
    const result = await (0, client_1.query)(`
    ...
    logger_1.logger.info("cleaner_preferences_set", { cleanerId });
    return result.rows[0];
}
// ============================================
// Availability Checks
 ...
 */
async function isCleanerAvailable(cleanerId, datetime) {
    const result = await (0, client_1.query)(`SELECT is_cleaner_available($1, $2::TIMESTAMPTZ) as is_available`, [cleanerId, datetime.toISOString()]);
    return result.rows[0]?.is_available ?? false;
}
/**
 * Check if a cleaner is available for a job slot (start to end)
 */
async function isCleanerAvailableForSlot(cleanerId, startAt, endAt) {
    ...
    const jobCountResult = await (0, client_1.query)(`
    ...
    if (existingJobs >= maxJobs)
        return false;
    // Check for overlapping jobs
    const overlapResult = await (0, client_1.query)(`
      SELECT COUNT(*) as count FROM jobs
      WHERE cleaner_id = $1
        AND status NOT IN ('cancelled', 'completed')
        AND (
 ...
 */
async function getAvailableCleaners(startAt, endAt, zipCode) {
    // Get all active cleaners
    const cleanersResult = await (0, client_1.query)(`SELECT user_id FROM cleaner_profiles`);
    const availableCleanerIds = [];
 ...
 */
async function cleanerServesZipCode(cleanerId, zipCode) {
 ...
 */
async function getCleanerSchedule(cleanerId, date) {
    const dayOfWeek = date.getDay();
    const dateStr = date.toISOString().split("T")[0];
    // Get availability for that day
    const availabilityResult = await (0, client_1.query)(`SELECT * FROM cleaner_availability WHERE cleaner_id = $1 AND day_of_week = $2`, [cleanerId, dayOfWeek]);
    // Get time off for that date
    const timeOffResult = await (0, client_1.query)(`SELECT * FROM cleaner_time_off WHERE cleaner_id = $1 AND $2::DATE BETWEEN start_date AND end_date`, [cleanerId, dateStr]);
