...
exports.getSupplyDemandHeatmap = getSupplyDemandHeatmap;
exports.getTierDistribution = getTierDistribution;
exports.getRetentionCohorts = getRetentionCohorts;
...
async function getCreditMetrics() {
    ...
    const velocityResult = await (0, client_1.query)(`
...
async function getRateMetrics() {
    ...
    `);
    const workflowResult = await (0, client_1.query)(`SELECT COUNT(*)::text as failed FROM webhook_failures WHERE created_at >= NOW() - INTERVAL '7 days'`);
    const row = result.rows[0];
 ...
 */
async function getSupplyDemandHeatmap() {
    // Demand: Jobs scheduled per hour/day
    const demandResult = await (0, client_1.query)(`
      SELECT
    ...
    const supplyResult = await (0, client_1.query)(`
      SELECT
        h.hour::text,
...
async function getActiveAlerts() {
    const alerts = [];
    // Check stuck jobs
    const stuckJobsResult = await (0, client_1.query)(`SELECT COUNT(*)::text as count FROM jobs WHERE status = 'requested' AND created_at < NOW() - INTERVAL '24 hours'`);
    ...
    const failedPayoutsResult = await (0, client_1.query)(`SELECT COUNT(*)::text as count FROM payouts WHERE status = 'failed'`);
    ...
    // Check open disputes
    const openDisputesResult = await (0, client_1.query)(`SELECT COUNT(*)::text as count FROM disputes WHERE status = 'open'`);
    ...
    // Check fraud alerts
    const fraudResult = await (0, client_1.query)(`SELECT COUNT(*)::text as count FROM fraud_alerts WHERE status = 'open'`);
