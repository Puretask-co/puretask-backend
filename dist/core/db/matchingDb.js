...
exports.getCandidatesForJob = getCandidatesForJob;
exports.logMatchRecommendation = logMatchRecommendation;
exports.getMatchHistoryForJob = getMatchHistoryForJob;
exports.assignCleanerToJob = assignCleanerToJob;
exports.getJobWithClientProfile = getJobWithClientProfile;
 ...
 */
async function getCandidatesForJob(context, clientId) {
    ...
    const result = await (0, client_1.query)(`WITH candidate_cleaners AS (
      SELECT 
        u.id,
 ...
 */
async function logMatchRecommendation(data) {
    await (0, client_1.query)(`INSERT INTO match_recommendations (
 ...
 */
async function getMatchHistoryForJob(jobId) {
    const result = await (0, client_1.query)(`SELECT cleaner_id, match_score, rank, generated_at
     FROM match_recommendations
     WHERE job_id = $1
     ORDER BY rank ASC`, [String(jobId)]);
    return result.rows.map(row => ({
 ...
 */
async function assignCleanerToJob(jobId, cleanerId, matchScore) {
    await (0, client_1.query)(`UPDATE jobs
     SET cleaner_id = $2, status = 'accepted', updated_at = NOW()
     WHERE id = $1`, [String(jobId), String(cleanerId)]);
    logger_1.logger.info("cleaner_assigned_to_job", {
        jobId,
        cleanerId,
        matchScore,
    });
 ...
 */
async function getJobWithClientProfile(jobId) {
