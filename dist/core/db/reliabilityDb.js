...
exports.getCleanerMetricsById = getCleanerMetricsById;
exports.sumCleanerEventWeightsSince = sumCleanerEventWeightsSince;
exports.countCleanerWeeklyStreaks = countCleanerWeeklyStreaks;
exports.updateCleanerWeeklyStreak = updateCleanerWeeklyStreak;
exports.updateCleanerReliability = updateCleanerReliability;
exports.countJobsForCleaner = countJobsForCleaner;
exports.insertCleanerEvent = insertCleanerEvent;
exports.getActiveCleaners = getActiveCleaners;
exports.refreshCleanerMetrics = refreshCleanerMetrics;
 ...
 */
async function getCleanerMetricsById(cleanerId) {
    // First try to get from pre-aggregated table
    const cached = await (0, client_1.query)(`SELECT * FROM cleaner_metrics WHERE cleaner_id = $1`, [String(cleanerId)]);
 ...
 */
async function computeCleanerMetrics(cleanerId) {
    const result = await (0, client_1.query)(`WITH recent_jobs AS (
      SELECT 
        j.*,
    ...
    FROM recent_jobs`, [String(cleanerId)]);
 ...
 */
async function upsertCleanerMetrics(metrics) {
    await (0, client_1.query)(`INSERT INTO cleaner_metrics (
 ...
 */
async function sumCleanerEventWeightsSince(cleanerId, days) {
    const result = await (0, client_1.query)(`SELECT COALESCE(SUM(weight), 0)::text as sum
     FROM cleaner_events
     WHERE cleaner_id = $1
     AND created_at >= NOW() - INTERVAL '1 day' * $2`, [String(cleanerId), days]);
 ...
 */
async function countCleanerWeeklyStreaks(cleanerId, maxWeeks = 5) {
    const result = await (0, client_1.query)(`SELECT COUNT(*)::text as count
     FROM cleaner_weekly_streaks
     WHERE cleaner_id = $1
     AND is_streak = true
     ORDER BY week_start DESC
     LIMIT $2`, [String(cleanerId), maxWeeks]);
 ...
 */
async function updateCleanerWeeklyStreak(cleanerId, weekStart) {
    ...
    const result = await (0, client_1.query)(`SELECT
     ...
     AND scheduled_start_at < $3`, [String(cleanerId), weekStart.toISOString(), weekEnd.toISOString()]);
    const row = result.rows[0] || {};
    const isStreak = Number(row.cancels || 0) === 0 &&
    ...
    await (0, client_1.query)(`INSERT INTO cleaner_weekly_streaks (cleaner_id, week_start, is_streak, created_at)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT (cleaner_id, week_start) DO UPDATE SET is_streak = $3`, [String(cleanerId), weekStart.toISOString().split('T')[0], isStreak]);
    return isStreak;
}
// ============================================
 ...
 */
async function updateCleanerReliability(cleanerId, data) {
    ...
    await (0, client_1.query)(`UPDATE cleaner_profiles
     ...
     WHERE user_id = $1`, [String(cleanerId), data.reliabilityScore, tierValue]);
    logger_1.logger.info("cleaner_reliability_updated", {
        cleanerId,
 ...
 */
async function countJobsForCleaner(cleanerId) {
    const result = await (0, client_1.query)(`SELECT COUNT(*)::text as count
     FROM jobs
     WHERE cleaner_id = $1
     AND status = 'completed'`, [String(cleanerId)]);
    return Number(result.rows[0]?.count || 0);
}
// ============================================
 ...
 */
async function insertCleanerEvent(event) {
    const result = await (0, client_1.query)(`INSERT INTO cleaner_events (cleaner_id, job_id, event_type, weight, metadata, created_at)
 ...
 */
async function getActiveCleaners() {
    const result = await (0, client_1.query)(`SELECT user_id FROM cleaner_profiles WHERE is_available = true`);
    return result.rows.map(row => ({ id: Number(row.user_id) }));
}
/**
 * Refresh metrics cache for a cleaner (called after job completion)
 */
async function refreshCleanerMetrics(cleanerId) {
