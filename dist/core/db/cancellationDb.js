...
exports.insertCancellationEvent = insertCancellationEvent;
exports.applyCancellationCredits = applyCancellationCredits;
exports.getClientGraceRemaining = getClientGraceRemaining;
exports.useGraceCancellation = useGraceCancellation;
exports.getClientCancellationStats = getClientCancellationStats;
exports.getCleanerCancellationStats = getCleanerCancellationStats;
exports.markClientNoShow = markClientNoShow;
exports.markCleanerNoShow = markCleanerNoShow;
 ...
 */
async function insertCancellationEvent(data) {
    ...
    logger_1.logger.info("cancellation_event_inserted", {
        id,
        jobId: data.jobId,
 ...
 */
async function applyCancellationCredits(data) {
    const { jobId, clientId, cleanerId, refundCredits, cleanerCompCredits, bonusCreditsToClient, } = data;
    // Refund credits to client
    if (refundCredits > 0) {
        await (0, client_1.query)(`INSERT INTO credit_ledger (user_id, job_id, delta_credits, reason, created_at)
       VALUES ($1, $2, $3, 'refund', NOW())`, [String(clientId), String(jobId), refundCredits]);
        logger_1.logger.info("credits_refunded", {
            clientId,
            jobId,
    ...
    if (cleanerCompCredits > 0) {
        await (0, client_1.query)(`INSERT INTO credit_ledger (user_id, job_id, delta_credits, reason, created_at)
       VALUES ($1, $2, $3, 'cancellation_compensation', NOW())`, [String(cleanerId), String(jobId), cleanerCompCredits]);
        logger_1.logger.info("cleaner_compensated", {
            cleanerId,
            jobId,
    ...
    if (bonusCreditsToClient && bonusCreditsToClient > 0) {
        await (0, client_1.query)(`INSERT INTO credit_ledger (user_id, job_id, delta_credits, reason, created_at)
       VALUES ($1, $2, $3, 'apology_bonus', NOW())`, [String(clientId), String(jobId), bonusCreditsToClient]);
        logger_1.logger.info("bonus_credits_issued", {
            clientId,
            jobId,
    ...
    await (0, client_1.query)(`UPDATE jobs SET status = 'cancelled', updated_at = NOW() WHERE id = $1`, [String(jobId)]);
}
// ============================================
 ...
 */
async function getClientGraceRemaining(clientId) {
 ...
 */
async function useGraceCancellation(clientId, jobId) {
    ...
    await (0, client_1.query)(`INSERT INTO grace_cancellations (client_id, job_id, created_at)
     VALUES ($1, $2, NOW())`, [String(clientId), jobId ? String(jobId) : null]);
 ...
 */
async function getClientCancellationStats(clientId, since) {
    const result = await (0, client_1.query)(`SELECT
 ...
 */
async function getCleanerCancellationStats(cleanerId, since) {
    const result = await (0, client_1.query)(`SELECT
      COUNT(*)::int as total_cancellations,
      COUNT(*) FILTER (WHERE bucket IN ('24_48', 'lt24'))::int as late_cancellations,
     ...
     AND t_cancel >= $2`, [String(cleanerId), since.toISOString()]);
    const row = result.rows[0] || {};
    return {
        totalCancellations: Number(row.total_cancellations || 0),
        lateCancellations: Number(row.late_cancellations || 0),
 ...
 */
async function markClientNoShow(jobId) {
    await (0, client_1.query)(`UPDATE jobs SET status = 'no_show_client', updated_at = NOW() WHERE id = $1`, [String(jobId)]);
    logger_1.logger.info("job_marked_client_no_show", { jobId });
}
/**
 * Mark a job as cleaner no-show
 */
async function markCleanerNoShow(jobId) {
    await (0, client_1.query)(`UPDATE jobs SET status = 'no_show_cleaner', updated_at = NOW() WHERE id = $1`, [String(jobId)]);
    logger_1.logger.info("job_marked_cleaner_no_show", { jobId });
}
