...
exports.insertRescheduleEvent = insertRescheduleEvent;
exports.updateRescheduleEvent = updateRescheduleEvent;
exports.findRescheduleEventById = findRescheduleEventById;
exports.countReschedulesForJob = countReschedulesForJob;
exports.countLateClientReschedulesLt24LastNDays = countLateClientReschedulesLt24LastNDays;
exports.updateJobStartTime = updateJobStartTime;
exports.isCleanerAvailableForRange = isCleanerAvailableForRange;
exports.logCleanerDeclineReasonableRequest = logCleanerDeclineReasonableRequest;
exports.getPendingReschedulesForJob = getPendingReschedulesForJob;
exports.getCleanerRescheduleStats = getCleanerRescheduleStats;
 ...
 */
async function insertRescheduleEvent(data) {
    ...
    logger_1.logger.info("reschedule_event_inserted", {
        id,
        jobId: data.jobId,
 ...
 */
async function updateRescheduleEvent(id, updates) {
 ...
 */
async function findRescheduleEventById(id) {
 ...
 */
async function countReschedulesForJob(jobId) {
 ...
 */
async function countLateClientReschedulesLt24LastNDays(clientId, days) {
    const result = await (0, client_1.query)(`SELECT COUNT(*)::text as count
     FROM reschedule_events
     WHERE client_id = $1
     AND requested_by = 'client'
     AND bucket = 'lt24'
     AND t_request >= NOW() - INTERVAL '1 day' * $2`, [String(clientId), days]);
    return Number(result.rows[0]?.count || 0);
}
// ============================================
 ...
 */
async function updateJobStartTime(jobId, newStartTime) {
    ...
    logger_1.logger.info("job_start_time_updated", {
        jobId,
 ...
 */
async function isCleanerAvailableForRange(cleanerId, startTime, endTime) {
    ...
    const conflictResult = await (0, client_1.query)(`SELECT id FROM jobs
     ...
     LIMIT 1`, [String(cleanerId), startTime.toISOString(), actualEndTime.toISOString()]);
 ...
 */
async function logCleanerDeclineReasonableRequest(data) {
 ...
 */
function mapRescheduleEventRow(row) {
    return {
        id: Number(row.id),
 ...
 */
async function getPendingReschedulesForJob(jobId) {
    const result = await (0, client_1.query)(`SELECT * FROM reschedule_events
     WHERE job_id = $1
     AND status = 'pending'
     ORDER BY created_at DESC`, [String(jobId)]);
    return result.rows.map(mapRescheduleEventRow);
}
/**
 * Get cleaner's reschedule stats for flexibility evaluation
 */
async function getCleanerRescheduleStats(cleanerId, since) {
    const requestsResult = await (0, client_1.query)(`SELECT COUNT(*)::text as count
     FROM reschedule_events
     WHERE cleaner_id = $1
     AND requested_to = 'cleaner'
     AND is_reasonable = true
     AND t_request >= $2`, [String(cleanerId), since.toISOString()]);
    const declinesResult = await (0, client_1.query)(`SELECT COUNT(*)::text as count
     FROM reschedule_events
     WHERE cleaner_id = $1
     AND requested_to = 'cleaner'
     AND is_reasonable = true
